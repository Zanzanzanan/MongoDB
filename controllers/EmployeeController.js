...
employeeController.save = async (req, res) => {
    try {
        ...
        if (isNaN(req.body.salary) || req.body.salary < 0) {
            return res.status(400).render("../views/employee/create", { 
                error: "Salary must be a valid number and cannot be negative."
            });
        }
...
employeeController.update = async (req, res) => {
    try {
        // Validate salary
        if (isNaN(req.body.salary) || req.body.salary < 0) {
            return res.status(400).render("../views/employee/edit", { 
                employee: req.body, 
                error: "Salary must be a valid number and cannot be negative."
            });
        }
        ...
        console.error(err);
        res.status(400).render("../views/employee/edit", { 
            employee: req.body, 
